            ...
            <div class="grid gap-6 px-5 py-6">
                <div class="grid gap-6 lg:grid-cols-2">
                    <div class="grid gap-4">
                        <div class="text-xs font-semibold uppercase tracking-[0.2em] text-muted">Penelitian</div>
                        <div
                            v-for="point in penelitianPoints"
                            :key="point.id"
                    ...
                    <div class="grid gap-4">
                        ...
                        <div
                            v-for="point in presentasiPoints"
                            :key="point.id"
                            class="rounded-2xl border border-border bg-white p-4"
                        >
                            <div class="text-sm font-semibold text-ink">
                        ...
                        <div class="text-xs font-semibold uppercase tracking-[0.2em] text-muted">
                            Kemampuan diskusi dan argumentasi
                        </div>
                        <div
                            v-for="point in diskusiPoints"
                            :key="point.id"
                            class="rounded-2xl border border-border bg-white p-4"
                        >
                            <div class="text-sm font-semibold text-ink">
...
export default {
    ...
    computed: {
        ...
        penelitianPoints() {
            return this.points.filter((point) => point.order < 9);
        },
        presentasiPoints() {
            return this.points.filter((point) => point.order > 8 && point.order < 12);
        },
        diskusiPoints() {
            return this.points.filter((point) => point.order > 11 && point.order < 15);
        },
    },
    created() {
        this.loadData();
    ...
    methods: {
        ...
        submitData() {
            ...
            return Repository.post(`/api/stase-task-logs-update-score-tesis/${this.form.stase_task_log_id}`, {
                params: this.form,
            })
                .then(() => {
                    Swal.fire({
