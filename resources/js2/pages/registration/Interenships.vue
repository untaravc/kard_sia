<template>
    <div class="rounded-2xl border border-border bg-panel p-6">
        <div class="flex items-center justify-between gap-4">
            <div class="text-lg font-semibold">Pengalaman Interenship</div>
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="text-xs uppercase text-muted">
                    <tr class="border-b border-border">
                        ...
                        <th class="px-2 py-2 hidden lg:table-cell">Periode</th>
                        <th class="px-2 py-2">Dokumen</th>
                        <th class="px-2 py-2 w-32 text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="interenships.length === 0">
                        <td colspan="5" class="px-2 py-6 text-center text-sm text-muted">Belum ada data</td>
                    </tr>
                    <tr v-for="(detail, index) in interenships" :key="detail.id" class="border-b border-border">
        ...
        <Modal :open="modalOpen" title="Pengalaman Interenship" :closeOnBackdrop="false" @close="closeModal">
...
export default {
    ...
    computed: {
        ...
        interenships() {
            ...
            return details.filter((item) => item && item.label === 'interenship');
        },
    },
    methods: {
        truncate(url) {
        ...
        showAddModal() {
            ...
            this.detailErrors = {};
            this.editMode = false;
            this.form = { id: null, label: 'interenship', name: '', year: '', file_url: '' };
            this.modalOpen = true;
        },
        showEditModal(detail) {
            ...
            this.form = {
                id: detail.id,
                label: detail.label || 'interenship',
                name: detail.name || '',
                year: detail.year || '',
        ...
        uploadFile(event) {
            ...
            this.uploading = true;
            uploadFirebaseFile({ file, prefix: 'Registration/Interenship' })
                .then((url) => {
                    if (url) {
