<template>
    <div class="rounded-2xl border border-border bg-panel p-6">
        <div class="flex items-center justify-between gap-4">
            <div class="text-lg font-semibold">Agenda Ilmiah</div>
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="text-xs uppercase text-muted">
                    <tr class="border-b border-border">
                        ...
                        <th class="px-2 py-2">Nama Acara/Kegiatan</th>
                        <th class="px-2 py-2 hidden lg:table-cell">Jenis Kegiatan</th>
                ...
                <tbody>
                    <tr v-if="scientifics.length === 0">
                        <td colspan="6" class="px-2 py-6 text-center text-sm text-muted">Belum ada data</td>
                    </tr>
                    <tr v-for="(detail, index) in scientifics" :key="detail.id" class="border-b border-border">
        ...
        <Modal :open="modalOpen" title="Agenda Ilmiah" :closeOnBackdrop="false" @close="closeModal">
            <div class="grid gap-4">
                <label class="grid gap-2 text-sm">
                    <span class="text-muted">Nama Acara/Kegiatan</span>
                ...
                <label class="grid gap-2 text-sm">
                    <span class="text-muted">Jenis Kegiatan</span>
                    <select class="w-full rounded-xl border bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2"
                        ...
                        <option value="Publikasi Karya Ilmiah di Seminar">Publikasi Karya Ilmiah di Seminar</option>
...
export default {
    ...
    computed: {
        ...
        scientifics() {
            ...
            return details.filter((item) => item && item.label === 'scientific');
        },
    },
    methods: {
        truncate(url) {
        ...
        showAddModal() {
            ...
            this.detailErrors = {};
            this.editMode = false;
            this.form = { id: null, label: 'scientific', name: '', desc: 'Penilitian', year: '', file_url: '' };
            this.modalOpen = true;
        },
        ...
        uploadFile(event) {
            ...
            this.uploading = true;
            uploadFirebaseFile({ file, prefix: 'Registration/Scientific' })
                .then((url) => {
                    if (url) {
