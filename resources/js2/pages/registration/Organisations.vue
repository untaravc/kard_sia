<template>
    <div class="rounded-2xl border border-border bg-panel p-6">
        <div class="flex items-center justify-between gap-4">
            <div class="text-lg font-semibold">Pengalaman Organisasi</div>
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="text-xs uppercase text-muted">
                    <tr class="border-b border-border">
                        ...
                        <th class="px-2 py-2 hidden lg:table-cell">Kegiatan</th>
                ...
                <tbody>
                    ...
                    <tr v-for="(detail, index) in organisations" :key="detail.id" class="border-b border-border">
        ...
        <Modal :open="modalOpen" title="Pengalaman Organisasi" :closeOnBackdrop="false" @close="closeModal">
            <div class="grid gap-4">
                <label class="grid gap-2 text-sm">
                    <span class="text-muted">Nama Organisasi</span>
...
export default {
    name: 'RegistrationOrganisations',
    components: { Modal },
    ...
    computed: {
        ...
        organisations() {
            ...
            return details.filter((item) => item && item.label === 'organisation');
        },
    },
    methods: {
        truncate(url) {
        ...
        showAddModal() {
            ...
            this.detailErrors = {};
            this.editMode = false;
            this.form = { id: null, label: 'organisation', name: '', desc: '', year: '', file_url: '' };
            this.modalOpen = true;
        },
        showEditModal(detail) {
            ...
            this.form = {
                id: detail.id,
                label: detail.label || 'organisation',
                name: detail.name || '',
                desc: detail.desc || '',
                year: detail.year || '',
        ...
        uploadFile(event) {
            ...
            this.uploading = true;
            uploadFirebaseFile({ file, prefix: 'Registration/Organisation' })
                .then((url) => {
                    if (url) {
