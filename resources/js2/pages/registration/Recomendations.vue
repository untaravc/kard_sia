    ...
    <div class="grid gap-4">
        <div class="rounded-2xl border border-border bg-panel p-6">
            ...
            <div class="mt-4 rounded-xl border border-border bg-surface px-4 py-3 text-sm text-slate-700">
                Rekomendasi berasal dari pihak akademik atau profesional yang mengetahui kemampuan, etika, dan komitmen dokter,
                baik secara pribadi maupun profesional. Pastikan pemberi rekomendasi memiliki jabatan kredibel dan bersedia
                dihubungi oleh pihak kami.
            </div>
                ...
                <table class="min-w-full text-left text-sm">
                    <thead class="text-xs uppercase text-muted">
                        <tr class="border-b border-border">
                            ...
                            <th class="px-2 py-2 w-[30%]">Nama</th>
                            <th class="px-2 py-2 hidden lg:table-cell">Jabatan</th>
                            <th class="px-2 py-2 hidden lg:table-cell">Alamat, Telepon</th>
                            <th class="px-2 py-2 hidden lg:table-cell">Hubungan dg Perekomendasi</th>
                    ...
                    <tbody>
                        <tr v-if="recommendations.length === 0">
                            <td colspan="7" class="px-2 py-6 text-center text-sm text-muted">Belum ada data</td>
                        </tr>
                        <tr v-for="(detail, index) in recommendations" :key="detail.id" class="border-b border-border">
                            <td class="px-2 py-2">{{ index + 1 }}</td>
                            <td class="px-2 py-2 font-semibold">{{ detail.name }}</td>
                            <td class="px-2 py-2 hidden lg:table-cell">{{ detail.desc }}</td>
                            <td class="px-2 py-2 hidden lg:table-cell">{{ detail.contact }}</td>
                            <td class="px-2 py-2 hidden lg:table-cell">{{ detail.desc_1 }}</td>
                            <td class="px-2 py-2">
        ...
        <div class="rounded-2xl border border-border bg-panel p-6">
            <div class="flex items-center justify-between gap-4">
                <div class="font-semibold">Unduh contoh dokumen rokomendasi</div>
        ...
        <Modal :open="modalOpen" title="Rekomendasi" :closeOnBackdrop="false" @close="closeModal">
            <div class="grid gap-4">
                ...
                <label class="grid gap-2 text-sm">
                    <span class="text-muted">Alamat, Telepon</span>
                    <input class="w-full rounded-xl border bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2"
                        :class="detailErrors.contact ? 'border-red-300 focus:ring-red-500/20' : 'border-border focus:ring-primary/30'"
                        v-model.trim="form.contact" />
                    <span v-if="detailErrors.contact" class="text-xs text-red-600">{{ detailErrors.contact }}</span>
                </label>

                <label class="grid gap-2 text-sm">
                    <span class="text-muted">Hubungan dg Perekomendasi</span>
                    <input class="w-full rounded-xl border bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2"
                        :class="detailErrors.desc_1 ? 'border-red-300 focus:ring-red-500/20' : 'border-border focus:ring-primary/30'"
                        v-model.trim="form.desc_1" />
                    <span v-if="detailErrors.desc_1" class="text-xs text-red-600">{{ detailErrors.desc_1 }}</span>
                </label>
...
export default {
    name: 'RegistrationRecomendations',
    components: { Modal },
    ...
    data() {
        return {
            ...
            referenceDocumentUrl:
                'https://firebasestorage.googleapis.com/v0/b/unt-dev.firebasestorage.app/o/KardiologiFkkmk%2FAssets%2Ftemplate_form_rekomendasi_jantung_dan_pembuluh_darah.docx?alt=media&token=83d0dba2-a6bd-4edb-b213-8690ff7dc16e',
        };
    },
    computed: {
        canEdit() {
        ...
        recommendations() {
            ...
            return details.filter((item) => item && item.label === 'recommendation');
        },
    },
    methods: {
        truncate(url) {
        ...
        showAddModal() {
            ...
            this.detailErrors = {};
            this.editMode = false;
            this.form = { id: null, label: 'recommendation', name: '', desc: '', contact: '', desc_1: '', file_url: '' };
            this.modalOpen = true;
        },
        showEditModal(detail) {
            ...
            this.form = {
                id: detail.id,
                label: detail.label || 'recommendation',
                name: detail.name || '',
                desc: detail.desc || '',
        ...
        uploadFile(event) {
            ...
            this.uploading = true;
            uploadFirebaseFile({ file, prefix: 'Registration/Recomendation' })
                .then((url) => {
                    if (url) {
