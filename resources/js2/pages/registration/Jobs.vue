<template>
    <div class="rounded-2xl border border-border bg-panel p-6">
        <div class="flex items-center justify-between gap-4">
            <div class="text-lg font-semibold">Riwayat Pekerjaan</div>
            ...
            <table class="min-w-full text-left text-sm">
                <thead class="text-xs uppercase text-muted">
                    <tr class="border-b border-border">
                        ...
                        <th class="px-2 py-2">Nama Tempat Kerja</th>
                        <th class="px-2 py-2 hidden lg:table-cell">Alamat</th>
                        <th class="px-2 py-2 hidden lg:table-cell">Jabatan</th>
                        <th class="px-2 py-2 hidden lg:table-cell">Tahun Masuk</th>
                        <th class="px-2 py-2 hidden lg:table-cell">Lama Bekerja</th>
                ...
                <tbody>
                    <tr v-if="jobs.length === 0">
                        <td colspan="8" class="px-2 py-6 text-center text-sm text-muted">Belum ada data</td>
                    </tr>
                    <tr v-for="(detail, index) in jobs" :key="detail.id" class="border-b border-border">
                        <td class="px-2 py-2">{{ index + 1 }}</td>
                        <td class="px-2 py-2 font-semibold">{{ detail.name }}</td>
                        <td class="px-2 py-2 hidden lg:table-cell">{{ detail.place }}</td>
                        <td class="px-2 py-2 hidden lg:table-cell">{{ detail.desc }}</td>
                        <td class="px-2 py-2 hidden lg:table-cell">{{ detail.year }}</td>
                        <td class="px-2 py-2 hidden lg:table-cell">{{ detail.duration }}</td>
                        <td class="px-2 py-2">
        ...
        <Modal :open="modalOpen" title="Riwayat Pekerjaan" :closeOnBackdrop="false" @close="closeModal">
            <div class="grid gap-4">
                <label class="grid gap-2 text-sm">
                    <span class="text-muted">Nama Tempat Kerja</span>
                ...
                <label class="grid gap-2 text-sm">
                    ...
                    <input class="w-full rounded-xl border bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2"
                        :class="detailErrors.place ? 'border-red-300 focus:ring-red-500/20' : 'border-border focus:ring-primary/30'"
                        v-model.trim="form.place" />
                    <span v-if="detailErrors.place" class="text-xs text-red-600">{{ detailErrors.place }}</span>
                </label>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <label class="grid gap-2 text-sm">
                        ...
                        <input class="w-full rounded-xl border bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2"
                            :class="detailErrors.duration ? 'border-red-300 focus:ring-red-500/20' : 'border-border focus:ring-primary/30'"
                            v-model.trim="form.duration" />
                        <span v-if="detailErrors.duration" class="text-xs text-red-600">{{ detailErrors.duration }}</span>
                    </label>
                </div>
...
export default {
    ...
    computed: {
        ...
        jobs() {
            ...
            return details.filter((item) => item && item.label === 'job');
        },
    },
    methods: {
        truncate(url) {
        ...
        uploadFile(event) {
            ...
            this.uploading = true;
            uploadFirebaseFile({ file, prefix: 'Registration/Jobs' })
                .then((url) => {
                    if (url) {
