                            ...
                            <div class="table-responsive mailbox-messages">
                                <table class="table table-hover table-striped">
                                    ...
                                    <tr v-for="(data, i) in dataContent.data" :key="data.id">
                                        ...
                                        <td>
                                            <span class="badge badge-success" v-if="data.type === 'send'">{{data.type}}</span>
                                            <span class="badge badge-info" v-if="data.type !== 'send'">{{data.type}}</span>
                                            <br>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-control" v-model="form.category" :class="{ 'is-invalid': form.errors.has('category') }">
                                    <option v-for="cat in dataRaw.categories" :value="cat.name">{{cat.desc}}</option>
                                </select>
    ...
    export default {
        ...
        methods:{
            loadDataContent(page = 1){
                ...
                this.isLoading = true;
                axios.get('/cmsd/documents', {params: this.filter})
                    .then( ({data}) => {
            ...
            loadCategories(type){
                axios.get('/cmsd/document_categories', {params: {type: type}})
                .then(({data})=>{
            ...
            createData() {
                ...
                axios.post( '/cmsd/documents', formData)
                    .then(({data}) => {
                        if(data.status){
            ...
            updateData(){
                this.disable = true;
                this.form.patch('/cmsd/documents/'+this.form.id)
                    .then((response) => {
            ...
            deleteData(id){
                Swal.fire({
                    ...
                    if (result.value) {
                        axios.post('/cmsd/documents/' + id, {_method: 'delete'}).then((response) => {
