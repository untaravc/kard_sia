...
<script>
    ...
    export default {
        data(){
            return {
                base_url: '/cmsd/student-score/'+this.$route.params.id,
                editMode : false,
                dataDetail: {},
                dataPoints:{},
                dataPointNote:'',
        ...
        methods:{
            ...
            openStase(){
                this.$Progress.start();
                this.form.post('/cmsd/open-stase-task')
                    .then((response) => {
            ...
            closeExam(data){
                if(confirm('Tutup sesi Ujian?')){
                    axios.get('/cmsd/close-stase-task', {params: data.open_stase_task[0]})
                        .then( ({response}) => {
                            Fire.$emit('ModalSuccess');
                        })
                }
            ...
            updateData(){
                this.$Progress.start();
                this.form.patch("/cmsd/stase-logs/"+this.form.id)
                    .then((response) => {
                        Fire.$emit('ModalSuccess');
                    }).catch((error) => {
                    console.log(error)
